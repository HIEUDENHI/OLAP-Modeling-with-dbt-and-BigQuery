-- back compat for old kwarg name
    ...
    merge into `data-435516`.`India`.`promo_code_dim` as DBT_INTERNAL_DEST
        using (
...
-- Incremental logic: Only insert rows where `promo_code_id_pk` does not already exist in the target table
    ...
    WHERE to_hex(md5(cast(coalesce(cast(Promotion_Code as STRING), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(Country as STRING), '_dbt_utils_surrogate_key_null_') || '-' || coalesce(cast(Region as STRING), '_dbt_utils_surrogate_key_null_') as STRING))) NOT IN (
        ...
        on (
                DBT_INTERNAL_SOURCE.promo_code_id_pk = DBT_INTERNAL_DEST.promo_code_id_pk
            )

    
    when matched then update set
        `promo_code_id_pk` = DBT_INTERNAL_SOURCE.`promo_code_id_pk`,`Promotion_Code` = DBT_INTERNAL_SOURCE.`Promotion_Code`,`Country` = DBT_INTERNAL_SOURCE.`Country`,`Region` = DBT_INTERNAL_SOURCE.`Region`,`is_active` = DBT_INTERNAL_SOURCE.`is_active`
    

    when not matched then insert
        (`promo_code_id_pk`, `Promotion_Code`, `Country`, `Region`, `is_active`)
    values
        (`promo_code_id_pk`, `Promotion_Code`, `Country`, `Region`, `is_active`)
