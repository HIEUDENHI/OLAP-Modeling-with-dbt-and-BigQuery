-- back compat for old kwarg name
    ...
    merge into `data-435516`.`India`.`date_dim` as DBT_INTERNAL_DEST
        using (

WITH combined_data AS (
    -- Combine the order_dt columns from all three staging tables
    SELECT Order_Date FROM `data-435516`.`India`.`int_sales_fr`
    UNION ALL
    SELECT Order_Date FROM `data-435516`.`India`.`int_sales_us`
    UNION ALL
    SELECT Order_Date FROM `data-435516`.`India`.`int_sales_in`
),
...
WHERE Order_Date > (
    SELECT MAX(Order_Date) FROM `data-435516`.`India`.`date_dim`
)
    ...
    when not matched then insert
        (`Order_Date`, `order_year`, `order_month`, `order_quarter`, `order_day`, `order_dayofweek`, `order_dayname`, `order_dayofmonth`, `order_weekday`, `day_counter`)
    values
        (`Order_Date`, `order_year`, `order_month`, `order_quarter`, `order_day`, `order_dayofweek`, `order_dayname`, `order_dayofmonth`, `order_weekday`, `day_counter`)
